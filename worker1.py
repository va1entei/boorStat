...
for i in boorDict2:
    ...
    popup_text = popup_text.format(i.split('Республика Беларусь,')[1],
                            str(int(float(boorDict0_1[i])*cdv)),
                            str(int(float(boorDict1[i])*cdv))
                            )
    if int(float(boorDict1[i])*cdv) ==0 or int(float(boorDict0_1[i])*cdv)/int(float(boorDict1[i])*cdv)>2:
          folium.CircleMarker(location = boorDict2[i], radius=float(boorDict0_1[i]),popup= popup_text, color='green', fill =False).add_to(world_map)
    else:
          folium.CircleMarker(location = boorDict2[i], radius=float(boorDict0_1[i]),popup= popup_text, color='blue', fill =False).add_to(world_map)

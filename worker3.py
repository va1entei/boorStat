...
ray = ''
for i in df.values:
    if 'ОБЛАСТНАЯ ОРГАНИЗАЦИОННАЯ СТРУКТУРА РГОО' in str(i[0]):
        obl = 'Республика Беларусь,'+i[0].split(' ')[1]+' область,'
    if i[1] in ['лось','олень','косуля'] and str(i[0]) != 'nan':
        ray = i[0].strip()[:-2]+'ий район'
    if i[1] == 'лось':
        boorDict1_elk[obl+ray] = (nz(i[2])-nz(i[3]))/cdv
        boorDict0_elk[obl+ray] = (nz(i[2])-nz(i[4])-nz(i[7]))/cdv
    if i[1] == 'олень':
        boorDict1_deer[obl+ray] = (nz(i[2])-nz(i[3]))/cdv
        boorDict0_deer[obl+ray] = (nz(i[2])-nz(i[4])-nz(i[7]))/cdv
    if i[1] == 'косуля':
        boorDict1_roe[obl+ray] = (nz(i[2])-nz(i[3]))/cdv
        boorDict0_roe[obl+ray] = (nz(i[2])-nz(i[4])-nz(i[7]))/cdv
